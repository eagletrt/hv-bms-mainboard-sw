...
#define ERROR_HANDLER_ERROR_INSTANCE_COUNT 2
 ...
 */
#define ERROR_HANDLER_ERROR_TOGGLE_IF(condition, group, instance, timestamp) \
    ((condition) ? error_handler_error_set(group, instance, timestamp) : error_handler_error_reset(group, instance))

/**
 * @brief Type of the error that categorize a group of instances
 *
 * @details
 *     - ERROR_HANDLER_ERROR_GROUP_POST the Power On Self Test procedure failed
 *     - ERROR_HANDLER_ERROR_GROUP_OVER_CURRENT no description
 */
typedef enum {
    ERROR_HANDLER_ERROR_GROUP_POST,
    ERROR_HANDLER_ERROR_GROUP_OVER_CURRENT,
    ERROR_HANDLER_ERROR_GROUP_COUNT
} ErrorHandlerErrorGroup;

/** @brief Single error instance type definition */
typedef uint16_t ErrorHandlerErrorInstance;
