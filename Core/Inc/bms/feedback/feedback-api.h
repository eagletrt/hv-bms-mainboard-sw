 ...
 */
enum FeedbackReturnCode feedback_init(feedback_read_digital_all_callback read_all, feedback_start_analog_conversion_callback start_conversion);
 ...
 * \retval FEEDBACK_OK otherwise
 */
enum FeedbackReturnCode feedback_update_analog_feedback(enum FeedbackAnalogIndex index, volt_t value);
 ...
 */
bool feedback_get_digital(enum FeedbackDigitalBit bit);
 ...
 */
volt_t feedback_get_analog(enum FeedbackAnalogIndex index);
 ...
 */
bool feedback_check_values(bit_flag32_t mask, bit_flag32_t value, enum FeedbackId *out);
