 ...
 * \retval LED_RC_OK otherwise
 */
enum LedReturnCode led_set_status(enum LedId led_id, enum LedStatus status);
 ...
 * \retval LED_RC_OK otherwise
 */
enum LedReturnCode led_toggle_status(enum LedId led_id);

#else // CONF_LED_MODULE_ENABLE

#define led_init(set, toggle) (LED_RC_OK)
#define led_set_status(id, status) (LED_RC_OK)
#define led_toggle_status(id) (LED_RC_OK)
