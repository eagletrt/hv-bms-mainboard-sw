 ...
 */
GPIO_TypeDef *_gpio_get_port_from_pcu_pin(const enum PcuPin pin) {
    switch (pin) {
 ...
 */
int16_t _gpio_get_pin_from_pcu_pin(const enum PcuPin pin) {
    switch (pin) {
        case PCU_PIN_AIR_NEGATIVE:
            return AIRN_OFF_Pin;
        case PCU_PIN_AIR_POSITIVE:
...
bit_flag32_t gpio_feedback_read_all(void) {
    ...
    feedbacks = MAINBOARD_BIT_TOGGLE_IF(feedbacks, HAL_GPIO_ReadPin(PLAUSIBLE_STATE_PERSISTED_GPIO_Port, PLAUSIBLE_STATE_PERSISTED_Pin) == GPIO_PIN_SET, FEEDBACK_DIGITAL_BIT_PLAUSIBLE_STATE_PERSISTED);
    ...
    feedbacks = MAINBOARD_BIT_TOGGLE_IF(feedbacks, HAL_GPIO_ReadPin(PLAUSIBLE_STATE_LATCHED_GPIO_Port, PLAUSIBLE_STATE_LATCHED_Pin) == GPIO_PIN_SET, FEEDBACK_DIGITAL_BIT_PLAUSIBLE_STATE_LATCHED);
