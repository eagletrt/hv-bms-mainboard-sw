...
_STATIC struct ImdHandler himd;

enum ImdReturnCode imd_init(const imd_pwm_start_callback start) {
    memset(&himd, 0U, sizeof(himd));
    himd.start = start;
    const enum ImdReturnCode code = (const enum ImdReturnCode)ir1553204_init(&himd.ir1153204);
    start();
    return code;
}
...
enum ImdStatus imd_get_status() {
    return (enum ImdStatus)ir1553204_get_status(&himd.ir1153204);
}
...
_EAGLETRT_STATIC char *imd_status_name[] = {
