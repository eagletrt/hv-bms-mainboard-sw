...
enum LoggerReturnCode logger_api_init(struct PalHandler *pal_h, enum LoggerLevel logger_state) {
    if (pal_h == NULL) {
...
void logger_api_set_state(enum LoggerLevel logger_state) {
    logger_handler.logger_state = logger_state;
}
...
enum LoggerReturnCode logger_api_log(enum LoggerLevel level, const char *format, ...) {
    ...
    if (level < logger_handler.logger_state) {
        return LOGGER_RC_OK; // Skip logging for non-error levels
    }
