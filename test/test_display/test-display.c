...
void test_display_init_ok(void) {
    enum DisplayReturnCode rc = display_init(display_set, display_toggle);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_OK, rc, "display_init should return DISPLAY_RC_OK");
...
void test_display_init_null_pointer_set(void) {
    enum DisplayReturnCode rc = display_init(NULL, display_toggle);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_NULL_POINTER, rc, "display_init should return DISPLAY_RC_NULL_POINTER when set is NULL");
}

void test_display_init_null_pointer_toggle(void) {
    enum DisplayReturnCode rc = display_init(display_set, NULL);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_NULL_POINTER, rc, "display_init should return DISPLAY_RC_NULL_POINTER when toggle is NULL");
}
...
void test_display_get_code_from_hex_digit_invalid(void) {
    enum DisplayCharacterCode code = display_get_code_from_hex_digit(16);
...
void test_display_get_segment_invalid(void) {
    enum DisplaySegmentStatus status = display_get_segment(DISPLAY_SEGMENT_INVALID);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_SEGMENT_STATUS_UNKNOWN, status, "display_get_segment should return DISPLAY_SEGMENT_STATUS_UNKNOWN for invalid segment");
}

void test_display_set_segment_invalid_segment(void) {
    enum DisplayReturnCode rc = display_set_segment(DISPLAY_SEGMENT_INVALID, DISPLAY_SEGMENT_STATUS_ON);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_INVALID_SEGMENT, rc, "display_set_segment should return DISPLAY_RC_INVALID_SEGMENT for invalid segment");
}

void test_display_set_segment_invalid_status(void) {
    enum DisplayReturnCode rc = display_set_segment(DISPLAY_SEGMENT_TOP, DISPLAY_SEGMENT_STATUS_UNKNOWN);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_INVALID_STATUS, rc, "display_set_segment should return DISPLAY_RC_INVALID_STATUS for invalid status");
}

void test_display_set_segment_ok(void) {
    enum DisplayReturnCode rc = display_set_segment(DISPLAY_SEGMENT_TOP, DISPLAY_SEGMENT_STATUS_ON);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_OK, rc, "display_set_segment should return DISPLAY_RC_OK for valid segment and status");
...
void test_display_toggle_segment_invalid_segment(void) {
    enum DisplayReturnCode rc = display_toggle_segment(DISPLAY_SEGMENT_INVALID);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_INVALID_SEGMENT, rc, "display_toggle_segment should return DISPLAY_RC_INVALID_SEGMENT for invalid segment");
}

void test_display_toggle_segment_ok(void) {
    // Set the segment to a known state
    display_set_segment(DISPLAY_SEGMENT_TOP, DISPLAY_SEGMENT_STATUS_OFF);
    // Toggle the segment
    enum DisplayReturnCode rc = display_toggle_segment(DISPLAY_SEGMENT_TOP);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_OK, rc, "display_toggle_segment should return DISPLAY_RC_OK for valid segment");
    TEST_ASSERT_TRUE_MESSAGE(display_set_fake.call_count > 0, "Expected call to display_set_fake was not made");
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_SEGMENT_TOP, display_set_fake.arg0_val, "display_set_fake was called with incorrect segment");
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_SEGMENT_STATUS_ON, display_set_fake.arg1_val, "display_set_fake was called with incorrect status");
}
...
void test_display_set_segment_all_ok(void) {
    ...
    enum DisplayReturnCode rc = display_set_segment_all(code);
...
void test_display_run_animation_null_pointer(void) {
    enum DisplayReturnCode rc = display_run_animation(NULL, 1, 100, 100);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_NULL_POINTER, rc, "display_run_animation should return DISPLAY_RC_NULL_POINTER when animation is NULL");
}
...
void test_display_run_animation_ok(void) {
    ...
    enum DisplayReturnCode rc = display_run_animation(animation, sizeof(animation) / sizeof(animation[0]), 100, 100);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_OK, rc, "display_run_animation should return DISPLAY_RC_OK for valid animation");
}

void test_display_run_animation_string_null_pointer(void) {
    enum DisplayReturnCode rc = display_run_animation(NULL, 0, 100, 100);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_NULL_POINTER, rc, "display_run_animation should return DISPLAY_RC_NULL_POINTER when animation is NULL");
}

void test_display_run_animation_string_ok(void) {
    const char *animation = "TOP MIDDLE BOTTOM";
    enum DisplayReturnCode rc = display_run_animation((const enum DisplaySegmentBit *)animation, strlen(animation), 100, 100);
    TEST_ASSERT_EQUAL_MESSAGE(DISPLAY_RC_OK, rc, "display_run_animation should return DISPLAY_RC_OK for valid animation string");
}

void setUp() {
    RESET_FAKE(display_set);
    RESET_FAKE(display_toggle);
    display_init(display_set, display_toggle);
}
