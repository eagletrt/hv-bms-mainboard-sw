...
void test_feedback_api_init_returns_null_pointer_when_read_callback_is_null(void) {
    enum FeedbackReturnCode result = feedback_api_init(NULL, feedback_start_analog_conversion);
...
void test_feedback_api_init_returns_null_pointer_when_start_conversion_callback_is_null(void) {
    enum FeedbackReturnCode result = feedback_api_init(feedback_read_digital_all, NULL);
...
void test_feedback_api_init_saves_callbacks_and_returns_ok(void) {
    enum FeedbackReturnCode result = feedback_api_init(feedback_read_digital_all, feedback_start_analog_conversion);
...
void test_feedback_update_digital_stores_read_result(void) {
    ...
    enum FeedbackReturnCode result = feedback_api_update_digital_feedback_all();

    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_RC_OK, result, "Expected OK return code from digital update");
    TEST_ASSERT_TRUE_MESSAGE(feedback_handler.digital & (1UL << FEEDBACK_DIGITAL_BIT_AIRN_OPEN_COM), "AIRN_OPEN_COM bit not set in digital field");
...
void test_feedback_api_update_analog_feedback_stores_value(void) {
    enum FeedbackReturnCode result = feedback_api_update_analog_feedback(FEEDBACK_ANALOG_INDEX_IMD_OK, 2.5f);
...
void test_feedback_api_update_analog_feedback_returns_invalid_index_when_out_of_range(void) {
    enum FeedbackReturnCode result = feedback_api_update_analog_feedback((enum FeedbackAnalogIndex)FEEDBACK_ANALOG_INDEX_COUNT, 1.0f);
...
void test_feedback_api_get_analog_status_probing_3v3_returns_high_when_in_range(void) {
    ...
    enum FeedbackStatus result = prv_feedback_api_get_analog_status(FEEDBACK_ANALOG_INDEX_PROBING_3V3);
...
void test_feedback_api_get_analog_status_returns_low_when_below_low_threshold(void) {
    ...
    enum FeedbackStatus result = prv_feedback_api_get_analog_status(FEEDBACK_ANALOG_INDEX_TSAL_GREEN);
...
void test_feedback_api_update_status_sets_high_for_set_digital_bits(void) {
    feedback_handler.digital = (1UL << FEEDBACK_DIGITAL_BIT_AIRN_OPEN_COM) |
    ...
    TEST_ASSERT_EQUAL_MESSAGE(expected[FEEDBACK_ID_AIRN_OPEN_COM], feedback_handler.status[FEEDBACK_ID_AIRN_OPEN_COM], "AIRN_OPEN_COM status should be HIGH when its digital bit is set");
    TEST_ASSERT_EQUAL_MESSAGE(expected[FEEDBACK_ID_SD_BMS_FB], feedback_handler.status[FEEDBACK_ID_SD_BMS_FB], "SD_BMS_FB status should be HIGH when its digital bit is set");
}
...
void test_feedback_api_update_status_sets_low_for_cleared_digital_bits(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_STATUS_LOW, feedback_handler.status[FEEDBACK_ID_AIRN_OPEN_COM], "AIRN_OPEN_COM status should be LOW when its digital bit is cleared");
    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_STATUS_LOW, feedback_handler.status[FEEDBACK_ID_AIRP_OPEN_COM], "AIRP_OPEN_COM status should be LOW when its digital bit is cleared");
}
...
void test_feedback_api_update_status_sets_high_for_analog_above_threshold(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_STATUS_HIGH, feedback_handler.status[FEEDBACK_ID_TSAL_GREEN], "TSAL_GREEN status should be HIGH when analog value exceeds high threshold");
}
...
void test_feedback_api_update_status_sets_low_for_analog_below_threshold(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_STATUS_LOW, feedback_handler.status[FEEDBACK_ID_SD_OUT], "SD_OUT status should be LOW when analog value is below low threshold");
}

// Feedback check values tests
void test_feedback_api_check_values_returns_true_and_sets_unknown_when_all_match(void) {
    ...
    bool result = feedback_api_check_values(mask, value, &out);
...
void test_feedback_api_check_values_returns_false_and_sets_failing_id_on_mismatch(void) {
    feedback_handler.status[FEEDBACK_ID_AIRN_OPEN_COM] = FEEDBACK_STATUS_LOW;
...
void test_feedback_api_check_values_skips_feedbacks_not_in_mask(void) {
    feedback_handler.status[FEEDBACK_ID_AIRN_OPEN_COM] = FEEDBACK_STATUS_HIGH; /* would fail if checked */
    feedback_handler.status[FEEDBACK_ID_SD_IMD_FB] = FEEDBACK_STATUS_HIGH;
...
void test_feedback_api_check_values_accepts_null_out_pointer(void) {
    ...
    bool result = feedback_api_check_values(mask, value, NULL);

    TEST_ASSERT_FALSE_MESSAGE(result, "Expected false on mismatch even when out pointer is NULL");
}

// Test getter invalid
void test_feedback_api_get_digital_bit_from_id_returns_unknown_for_invalid_id(void) {
    enum FeedbackDigitalBit result = feedback_api_get_digital_bit_from_id((enum FeedbackId)FEEDBACK_ID_COUNT);

    TEST_ASSERT_EQUAL_MESSAGE(FEEDBACK_DIGITAL_BIT_UNKNOWN, result, "Expected UNKNOWN digital bit for out-of-range feedback id");
}

void test_feedback_api_get_analog_index_from_id_returns_unknown_for_invalid_id(void) {
    enum FeedbackAnalogIndex result = feedback_api_get_analog_index_from_id((enum FeedbackId)FEEDBACK_ID_COUNT);
...
void test_feedback_api_get_status_payload_returns_correct_pointer_and_size(void) {
    ...
    feedback_handler.status[FEEDBACK_ID_SD_BMS_FB] = FEEDBACK_STATUS_LOW;

    primary_hv_feedback_status_converted_t *payload = feedback_api_get_status_payload(&byte_size);

    TEST_ASSERT_EQUAL_PTR_MESSAGE(&feedback_handler.status_can_payload, payload, "Status payload pointer should point to internal CAN payload struct");
...
void test_feedback_api_get_digital_payload_returns_correct_pointer_and_size(void) {
    ...
    primary_hv_feedback_digital_converted_t *payload = feedback_api_get_digital_payload(&byte_size);
...
void test_feedback_api_get_analog_payload_returns_correct_pointer_and_size(void) {
    ...
    feedback_handler.analog[FEEDBACK_ANALOG_INDEX_V5_MCU] = 2.50f;

    primary_hv_feedback_analog_converted_t *payload = feedback_api_get_analog_payload(&byte_size);
...
void test_feedback_api_get_analog_sd_payload_returns_correct_pointer_and_size(void) {
    ...
    primary_hv_feedback_analog_sd_converted_t *payload = feedback_api_get_analog_sd_payload(&byte_size);
...
void test_feedback_api_get_enzomma_payload_returns_correct_pointer_and_size(void) {
    size_t byte_size = 0U;
...
void test_feedback_api_get_enzomma_payload_digital_feedback_sets_is_digital_and_digital_value(void) {
    feedback_handler.status[FEEDBACK_ID_AIRN_OPEN_COM] = FEEDBACK_STATUS_HIGH;
    feedback_handler.digital = (1UL << FEEDBACK_DIGITAL_BIT_AIRN_OPEN_COM);
...
void test_feedback_api_get_enzomma_payload_analog_feedback_sets_analog_value(void) {
    feedback_handler.status[FEEDBACK_ID_IMD_OK] = FEEDBACK_STATUS_LOW;
    feedback_handler.analog[FEEDBACK_ANALOG_INDEX_IMD_OK] = 2.22f;

    primary_hv_feedback_enzomma_converted_t *payload =
        feedback_api_get_enzomma_payload(FEEDBACK_ID_IMD_OK, NULL);
...
void setUp(void) {
    ...
    (void)feedback_api_init(feedback_read_digital_all, feedback_start_analog_conversion);
}
...
int main(void) {
    ...
    RUN_TEST(test_feedback_api_check_values_returns_true_and_sets_unknown_when_all_match);
    RUN_TEST(test_feedback_api_check_values_returns_false_and_sets_failing_id_on_mismatch);
    RUN_TEST(test_feedback_api_check_values_skips_feedbacks_not_in_mask);
    RUN_TEST(test_feedback_api_check_values_accepts_null_out_pointer);

    RUN_TEST(test_feedback_api_get_digital_bit_from_id_returns_unknown_for_invalid_id);
    RUN_TEST(test_feedback_api_get_analog_index_from_id_returns_unknown_for_invalid_id);
    ...
    RUN_TEST(test_feedback_api_get_enzomma_payload_returns_correct_pointer_and_size);
    RUN_TEST(test_feedback_api_get_enzomma_payload_digital_feedback_sets_is_digital_and_digital_value);
    RUN_TEST(test_feedback_api_get_enzomma_payload_analog_feedback_sets_analog_value);
