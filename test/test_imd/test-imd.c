...
void test_imd_update_ok(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(10, ir1553204_api_get_frequency(&imd_handler.ir1153204), "imd_api_update should set the frequency correctly");
    TEST_ASSERT_EQUAL_MESSAGE(0.5, ir1553204_api_get_duty_cycle(&imd_handler.ir1153204), "imd_api_update should set the duty cycle correctly");
}

void test_imd_get_status_canlib_payload_null(void) {
    const primary_hv_imd_status_converted_t *payload = imd_api_get_status_canlib_payload(NULL);
    TEST_ASSERT_NOT_NULL_MESSAGE(payload, "imd_api_get_status_canlib_payload should not return NULL");

    TEST_ASSERT_EQUAL_MESSAGE((primary_hv_imd_status_status)(imd_api_get_status() + 1U), payload->status, "imd_api_get_status_canlib_payload should set the status correctly");
    TEST_ASSERT_EQUAL_MESSAGE(imd_api_get_frequency(), payload->frequency, "imd_api_get_status_canlib_payload should set the frequency correctly");
    TEST_ASSERT_EQUAL_MESSAGE(imd_api_get_duty_cycle(), payload->duty_cycle, "imd_api_get_status_canlib_payload should set the duty cycle correctly");
}

void test_imd_get_status_canlib_payload_byte_size(void) {
    size_t byte_size = 0;
    const primary_hv_imd_status_converted_t *payload = imd_api_get_status_canlib_payload(&byte_size);
    TEST_ASSERT_NOT_NULL_MESSAGE(payload, "imd_api_get_status_canlib_payload should not return NULL");
    TEST_ASSERT_EQUAL_MESSAGE(sizeof(primary_hv_imd_status_converted_t), byte_size, "imd_api_get_status_canlib_payload should set the correct byte size");
